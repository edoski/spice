...
RUN_DIR = (
    ...
    / "benchmarks"
    / "runs"
    / "lstm_36s_wall_clock_quartile_eval"
    / "20260628T131456Z"
)
...
CHAIN_CONFIGS = (
    ChainConfig(
        ...
        scan_prefix="ethereum_pectra_jun20_wall_clock_quartile",
        figure_prefix="ethereum_pectra_jun20_lstm_wall_clock_quartile",
    ),
    ChainConfig(
        chain="polygon",
        label="Polygon Bhilai",
        scan_prefix="polygon_bhilai_large_lstm_wall_clock_quartile",
        figure_prefix="polygon_bhilai_large_lstm_wall_clock_quartile",
    ),
    ChainConfig(
        chain="avalanche",
        label="Avalanche Octane",
        scan_prefix="avalanche_octane_large_lstm_wall_clock_quartile",
        figure_prefix="avalanche_octane_large_lstm_wall_clock_quartile",
    ),
)
...
def window_metric_summary(metrics: Iterable[dict[str, object]], metric_id: str) -> tuple[float, float]:
    for metric in metrics:
        if metric["metric_id"] == metric_id:
...
def read_selected_rows(config: ChainConfig) -> dict[str, dict[str, str]]:
    path = SCAN_DIR / f"{config.scan_prefix}_windows_selected_source.csv"
...
def joined_rows() -> list[dict[str, object]]:
    ...
    for record in read_collection_records():
        ...
        profit_mean, profit_std = window_metric_summary(record["window_metrics"], "profit_over_baseline")
        hit_mean, hit_std = window_metric_summary(record["window_metrics"], "exact_optimum_hit_rate")
        repetitions = 50.0
        rows.append(
            {
                "chain": chain,
                ...
                "selection_quartile": scan_row["selection_quartile"],
...
def y_limits(values: list[float], errors: list[float], *, floor_zero: bool = False) -> tuple[float, float]:
...
def render_chain_scatter(
    *,
    ...
    if len(plot_rows) != 108:
        raise ValueError(f"{config.chain} {selection_metric}: expected 108 rows, got {len(plot_rows)}")
...
def render_cross_chain_profit_facets(
    ...
    *,
    ...
    for ax, config in zip(axes, CHAIN_CONFIGS, strict=True):
        plot_rows = [row for row in rows_by_chain[config.chain] if row["selection_metric"] == selection_metric]
        setup_axis(ax)
        scatter_points(
            ax,
            plot_rows,
    ...
    all_plot_rows = [row for rows in rows_by_chain.values() for row in rows if row["selection_metric"] == selection_metric]
    ...
    fig.suptitle("LSTM wall-clock quartile profit comparison", x=0.055, ha="left", fontweight="bold")
    fig.legend(
        handles=quartile_handles(),
        loc="lower center",
        ncol=4,
...
def summary_rows(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for chain in chain_values:
        chain_rows = rows if chain == "all_chains" else [row for row in rows if row["chain"] == chain]
        for selection_metric in ("fee", "volatility"):
            metric_rows = [row for row in chain_rows if row["selection_metric"] == selection_metric]
            for quartile in ("q1", "q2", "q3", "q4", "all"):
                group = metric_rows if quartile == "all" else [
                    row for row in metric_rows if row["selection_quartile"] == quartile
                ]
                if not group:
                    continue
...
def pearson_rows(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    chain_values = [config.chain for config in CHAIN_CONFIGS] + ["all_chains"]
    specs = (
        ("fee", "median_base_fee_gwei", "median base fee"),
        ("fee", "log10_median_base_fee_gwei", "log10 median base fee"),
...
def markdown_report(correlations: list[dict[str, object]], summaries: list[dict[str, object]]) -> str:
    config_by_chain = {config.chain: config for config in CHAIN_CONFIGS}
    lines: list[str] = []
    lines.append("### 29/06 wall-clock quartile LSTM results")
    lines.append("")
    lines.append("Wall-clock quartile rerun: 648 evaluation windows, 216 per chain. Each chain has 108 fee-quartile windows and 108 volatility-quartile windows.")
    lines.append("")
    lines.append("Figures use only the windows selected for the plotted x-axis: fee-selected windows for base-fee plots, volatility-selected windows for volatility plots. Point whiskers are 95% CI over the 50 Poisson replay repetitions.")
    lines.append("")
    ...
    lines.append("![[lstm_36s_wall_clock_quartile_cross_chain_profit_vs_base_fee.png]]")
    lines.append("![[lstm_36s_wall_clock_quartile_cross_chain_profit_vs_base_fee_volatility.png]]")
    lines.append("")
    ...
    lines.append("")
    lines.append("These CIs are over selected evaluation windows in each quartile class, not over individual Poisson arrivals.")
    lines.append("")
    lines.append("| Chain | Class axis | Quartile | Windows | Mean profit | 95% CI | Mean accuracy | 95% CI |")
    lines.append("| --- | --- | --- | ---: | ---: | ---: | ---: | ---: |")
    for row in summaries:
        ...
        axis = "base fee" if row["selection_metric"] == "fee" else "volatility"
        quartile = str(row["quartile"]).upper()
        lines.append(
            f"| {label} | {axis} | {quartile} | {row['windows']} | "
            f"{float(row['mean_profit_percent']):.3f}% | "
            f"[{float(row['ci95_profit_low_percent']):.3f}, {float(row['ci95_profit_high_percent']):.3f}] | "
            f"{float(row['mean_exact_optimum_hit_rate_percent']):.2f}% | "
            f"[{float(row['ci95_accuracy_low_percent']):.2f}, {float(row['ci95_accuracy_high_percent']):.2f}] |"
        )
    lines.append("")
    lines.append("Exports:")
    lines.append(f"- `{EXPORT_DIR / 'lstm_36s_wall_clock_quartile_joined.csv'}`")
    lines.append(f"- `{EXPORT_DIR / 'lstm_36s_wall_clock_quartile_correlations.csv'}`")
    lines.append(f"- `{EXPORT_DIR / 'lstm_36s_wall_clock_quartile_summary.csv'}`")
    lines.append("")
    lines.append("Remaining professor-facing work:")
    lines.append("- Implement the block-count window scanner and block-based Poisson replay evaluator.")
    lines.append("- Rerun the same quartile-balanced design with one fixed block count per chain.")
    lines.append("- Recompute these figures and correlations on block-based results; those should be the final cross-chain comparison.")
    return "\n".join(lines) + "\n"


def main() -> None:
    rows = joined_rows()
    write_csv(EXPORT_DIR / "lstm_36s_wall_clock_quartile_joined.csv", rows, CSV_FIELDS)

    rows_by_chain = {config.chain: [row for row in rows if row["chain"] == config.chain] for config in CHAIN_CONFIGS}
    for config in CHAIN_CONFIGS:
        write_csv(
    ...
    render_cross_chain_profit_facets(
        ...
        selection_metric="fee",
        x_key="median_base_fee_gwei",
        filename="lstm_36s_wall_clock_quartile_cross_chain_profit_vs_base_fee",
    )
    render_cross_chain_profit_facets(
        rows_by_chain,
        selection_metric="volatility",
        x_key="base_fee_volatility_log_change_std",
        filename="lstm_36s_wall_clock_quartile_cross_chain_profit_vs_base_fee_volatility",
    )

    correlations = pearson_rows(rows)
    summaries = summary_rows(rows)
    write_csv(EXPORT_DIR / "lstm_36s_wall_clock_quartile_correlations.csv", correlations)
    write_csv(EXPORT_DIR / "lstm_36s_wall_clock_quartile_summary.csv", summaries)

    report = markdown_report(correlations, summaries)
    (EXPORT_DIR / "lstm_36s_wall_clock_quartile_report.md").write_text(report, encoding="utf-8")

    print(f"joined_rows={len(rows)}")
    for config in CHAIN_CONFIGS:
        chain_rows = rows_by_chain[config.chain]
        fee_rows = sum(1 for row in chain_rows if row["selection_metric"] == "fee")
        volatility_rows = sum(1 for row in chain_rows if row["selection_metric"] == "volatility")
        print(f"{config.chain}: total={len(chain_rows)} fee={fee_rows} volatility={volatility_rows}")
    print(f"correlations={EXPORT_DIR / 'lstm_36s_wall_clock_quartile_correlations.csv'}")
    print(f"summary={EXPORT_DIR / 'lstm_36s_wall_clock_quartile_summary.csv'}")
    print(f"report={EXPORT_DIR / 'lstm_36s_wall_clock_quartile_report.md'}")
