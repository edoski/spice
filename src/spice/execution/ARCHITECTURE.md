...
`execution.transfer` owns Storage Transfer Transaction behavior. It receives an **Execution Session**, resolves remote records through the **Remote Catalog Record Codec**, derives canonical destination paths through catalog materialization, stages roots, invokes rsync, promotes with root-kind validation, and cleans failed stages without hiding the primary failure. Public callers use `push_dataset_to_cluster()` and `pull_artifact_from_cluster()`; the transaction mechanics stay private to the module.
