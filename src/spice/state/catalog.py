...
def _upsert(path: Path, table: Table, values: dict[str, object], *, key_column) -> None:
...
def _delete_row(path: Path, *, table: Table, key_column, key: str) -> None:
    ensure_catalog_db(path)
    engine = _create_engine(path)
    try:
        with engine.begin() as conn:
...
def _select_rows(path: Path, *, table: Table, filters: list[object], order_by: list[object]):
    ensure_catalog_db(path)
    engine = _create_engine(path)
    try:
        statement = select(table)
        active_filters = [condition for condition in filters if condition is not None]
        if active_filters:
