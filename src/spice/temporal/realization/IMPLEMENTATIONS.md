# Concrete Realization Policy

Realization policies convert decoded model actions into concrete block rows. Current code has one policy: `strict_deadline_miss`.

## Mental Model

The model predicts an offset. The realization policy decides what that offset means economically.
...
A new realization policy should define baseline, optimum, overflow behavior, and target construction together. Prediction families and evaluators should consume the policy through the shared realization interface.
