...
def test_collection_resolver_pulls_baseline_artifact_and_loads_matching_summary(
    ...
    assert pulled[0]["selector"].artifact_id is not None
    assert pulled[0]["replace"] is True


def test_collection_resolver_pulls_tuned_study_before_artifact(
    tmp_path: Path,
    monkeypatch,
    load_workflow_config,
) -> None:
    ...
    monkeypatch.setattr(
        "spice.benchmarks.collection_resolver.resolve_workflow_paths",
        lambda _config, **_kwargs: SimpleNamespace(
            study_id="study-1",
            artifact_id="artifact-1",
            artifact_state_db=tmp_path / "state.sqlite",
        ),
    )
    monkeypatch.setattr(
        "spice.benchmarks.collection_resolver.pull_study_from_cluster",
        lambda **_kwargs: calls.append("study"),
    )
    monkeypatch.setattr(
        "spice.benchmarks.collection_resolver.apply_study_best_params",
        lambda _config: SimpleNamespace(config=config, study_id="study-1"),
    )
    monkeypatch.setattr(
        "spice.benchmarks.collection_resolver.pull_artifact_from_cluster",
        lambda **_kwargs: calls.append("artifact") or (SimpleNamespace(), False),
    )
    monkeypatch.setattr(
        "spice.benchmarks.collection_resolver.load_training_summary",
        lambda _path: None,
    ...
    monkeypatch.setattr(
        "spice.benchmarks.collection_resolver.list_evaluation_summaries",
        lambda _path: [_summary(config)],
    )

    resolved = resolve_benchmark_evaluation(config, session=session)

    assert resolved is not None
    assert calls == ["study", "artifact"]
