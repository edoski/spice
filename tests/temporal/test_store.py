...
def test_temporal_store_uses_real_timestamps_for_context_and_candidates() -> None:
    ...
    np.testing.assert_array_equal(
        ...
        np.array([2, 1, 1, 2, 1], dtype=np.int64),
    )
...
def test_estimated_block_store_uses_corpus_calibration_for_row_geometry() -> None:
    ...
    np.testing.assert_array_equal(store.anchor_rows, np.array([1, 2, 3], dtype=np.int64))
    np.testing.assert_array_equal(store.context_start_rows, np.array([0, 1, 2], dtype=np.int64))
    np.testing.assert_array_equal(store.candidate_end_rows, np.array([5, 6, 7], dtype=np.int64))
    np.testing.assert_array_equal(store.candidate_counts, np.array([3, 3, 3], dtype=np.int64))
