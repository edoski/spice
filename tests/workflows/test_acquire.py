...
def test_acquire_failure_preserves_staging_and_rerun_resumes(
    ...
    monkeypatch.setattr("spice.workflows.acquire.BlockRpcClient", FakeAcquireClient)
    monkeypatch.setattr(
        "spice.corpus.assembly._count_valid_history_samples",
        ...
        async def get_block_rows(self, start: int, end: int):
            ...
            if first_block >= evaluation_plan.block_range.start:
                ...
                plan = next(
                    ...
                    if first_block >= plan.block_range.start
                )
            return make_block_rows(
                end - start,
                start_block=first_block,
...
def test_acquire_workflow_refills_missing_history_prefix(
    tmp_path,
    monkeypatch,
    load_workflow_config,
    acquire_override,
...
def test_acquire_workflow_fails_after_bounded_short_refills(
    tmp_path,
    monkeypatch,
    load_workflow_config,
    acquire_override,
